...
class Test_2a(GradedTestCase):
    ...
    @graded(timeout=1)
    def test_4(self):
        """2a-4-basic: Basic test case for checking your own prompt count and spacing is different from the ones we have shown."""
        ...
        for n_support in range(len(support_inputs)):
            ...
            # feel free to add your prompt here
            # assert prompt_qa.count('custom') == n_support + 1, "Prompt format for custom is incorrect"
...
class Test_3c(GradedTestCase):
    ...
    def test_0(self):
        ...
        # Check that AB^T is zero
        self.assertTrue(
            torch.equal(lora_layer.lora_A @ lora_layer.lora_B.T, torch.zeros((d1, d2))), 
            "`AB^T` should initially be 0! Please follow all requirements outlined in the function comments and the writeup."
        )
